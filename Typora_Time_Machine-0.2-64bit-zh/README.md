...
直接运行Typora_Time_Machine.exe，即可打开Typora。如果您的Typora不在默认位置，首次运行时会出现出错的弹窗，关闭弹窗，在程序中输入Typora的正确路径（如果路径含空格要加英文的引号，并且以“Typora.exe”结尾，而不是以它所在的文件夹结尾），然后程序会自动将正确的路径保存在path.txt中，以后就可以正常运行了。